...
const DEFAULT_RELEASES_API_URL: &str =
    "https://api.github.com/repos/Next-AGI-Org/Next-GrokApp/releases/latest";
const DEFAULT_RELEASES_HTML_URL: &str =
    "https://github.com/Next-AGI-Org/Next-GrokApp/releases/latest";
const DEFAULT_RELEASES_PAGE: &str = "https://github.com/Next-AGI-Org/Next-GrokApp/releases";
...
///
/// Accepts:
/// - `https://github.com/Next-AGI-Org/Next-GrokApp/releases/tag/v0.1.7`
/// - `.../releases/tag/v0.1.7?foo=1`
/// - `/Next-AGI-Org/Next-GrokApp/releases/tag/0.1.7`
...
async fn fetch_via_html_redirect(
    ...
    let ua = format!(
        "GrokApp/{current_version} (desktop; check-update; +https://github.com/Next-AGI-Org/Next-GrokApp)"
    );
    ...
    if status == 200 {
        ...
        if let Some(idx) = body.find("/releases/tag/") {
            ...
            if let Some(tag) = extract_tag_from_release_url(&format!("https://github.com{slice}")) {
                ...
                let html = format!("https://github.com/Next-AGI-Org/Next-GrokApp/releases/tag/{tag_path}");
...
pub async fn check_app_update() -> Result<AppUpdateCheck, String> {
    ...
    let ua = format!(
        "GrokApp/{current} (desktop; check-update; +https://github.com/Next-AGI-Org/Next-GrokApp)"
    );
...
mod tests {
    ...
    fn parse_github_release_update_and_same() {
        let sample = json!({
            ...
            "html_url": "https://github.com/Next-AGI-Org/Next-GrokApp/releases/tag/v0.2.0",
    ...
    fn extract_tag_from_release_url_ok() {
        ...
        assert_eq!(
            extract_tag_from_release_url(
                "https://github.com/Next-AGI-Org/Next-GrokApp/releases/tag/0.2.0?foo=1#sec"
            )
            .as_deref(),
            Some("0.2.0")
        );
        assert_eq!(
            extract_tag_from_release_url("/Next-AGI-Org/Next-GrokApp/releases/tag/v1.0.0").as_deref(),
            Some("v1.0.0")
        );
        assert!(extract_tag_from_release_url("https://github.com/Next-AGI-Org/Next-GrokApp/releases").is_none());
