...
本仓公开 README **不要**出现：作者推广、社交链接、微信/X 二维码、贡献者头像画廊、界面截图橱窗。  
同步 `upstream` 后若这些内容被写回，应立刻删掉，保持独立产品表述（见 `AGENTS.md` 1d）。

`scripts/update-contributors.py` 为上游遗留工具；**发版脚本不再调用**，也不要手跑来“刷新画廊”。
